...
## 🐳 Running with Docker (Recommended - Easiest & Fastest)

If you have **Docker Desktop** installed, you do not need to install MongoDB, Node.js, or run any npm install commands manually. Everything is containerized and ready to run with a single command.

### Steps to Run:
1. Open VS Code, select **File -> Open Folder...** and choose the extracted project folder.
2. Open a new terminal in VS Code (Shortcut: `Ctrl + \`` or `Ctrl + Shift + \``).
3. Ensure **Docker Desktop** is open and running in the background.
4. Run the following command in the terminal:
   ```bash
   docker-compose up --build
   ```
5. Wait for the containers to compile. Once the logs show the server is listening, open your browser and navigate to:
...
## 🛠 Manual Installation & Usage (Without Docker)
